...
(function initRuilingsPage() {
  ...
  async function loadData() {
    try {
      ...
      const deletedEntryIds = readLocalDeletedEntryIds();
      state.entries = (payload.entries || [])
        .map((entry) => normalizeEntryForState(entry))
        .filter((entry) => !deletedEntryIds.has(Number(entry.id || entry.serial)));
  ...
  async function confirmAndDeleteRuiling(entryId) {
    ...
    if (!API_BASE) {
      ...
      persistLocalDeletedEntryId(entryId);
      ...
      renderHeroStats();
      applyFiltersAndRender();
      showToast('Ruiling Deleted', 'Entry deleted from this local rebuilt copy.', 'warning');
      return;
    }
  ...
  function readLocalDeletedEntryIds() {
    try {
      const raw = window.localStorage?.getItem(LOCAL_DELETED_STORAGE_KEY);
      if (!raw) {
        return new Set();
      }
      const parsed = JSON.parse(raw);
      ...
      console.warn('Could not read local deleted entries.', error);
      return new Set();
    }
  }

  function writeLocalDeletedEntryIds(ids) {
    try {
      window.localStorage?.setItem(LOCAL_DELETED_STORAGE_KEY, JSON.stringify([...ids]));
    } catch (error) {
      console.warn('Could not write local deleted entries.', error);
    }
  }

  function persistLocalDeletedEntryId(entryId) {
    const id = Number(entryId || 0);
    if (!Number.isFinite(id) || id <= 0) {
      return;
    }
    const current = readLocalDeletedEntryIds();
    current.add(id);
    writeLocalDeletedEntryIds(current);
  }
